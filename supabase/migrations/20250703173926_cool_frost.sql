/*
  # Add Quiz Questions Table with Conditional Policy Creation
  ...
  2. Security
    ...
    - Add policies for users to access questions for their own courses only
    - Safely handle storage bucket and policy creation

  3. Notes
    - Uses conditional blocks to prevent duplicate policy errors
    - Only creates new quiz_questions table and related policies
    - Existing tables and policies are left unchanged
*/
...
BEGIN
  ...
  -- Policy for inserting questions
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
