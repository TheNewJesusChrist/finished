...
import { ArrowLeft, CheckCircle, XCircle, Brain, Trophy, RotateCcw, AlertCircle, RefreshCw, Loader, BookOpen, Lightbulb, Home } from 'lucide-react';
        ...
        <div className="min-h-screen bg-gradient-to-br from-[#F5F7FA] to-[#E1E8F0] pl-64 pt-16">
          <div className="max-w-4xl mx-auto p-8">
            <div className="bg-white rounded-2xl shadow-lg p-8 border border-[#CBD5E1] text-center">
              ...
              <h1 className="text-2xl font-bold text-[#2E3A59] mb-4">Quiz Error</h1>
              <p className="text-[#BFC9D9] mb-6">
                Something went wrong while loading the quiz. Please try refreshing the page or go back to courses.
              </p>
              <div className="flex justify-center space-x-4">
                ...
                <motion.button
                  ...
                  className="px-6 py-3 bg-gray-500 text-white rounded-lg font-semibold flex items-center space-x-2"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
...
const Quiz: React.FC = () => {
    ...
    try {
      const guestQuestions: Record<string, QuizQuestion[]> = {
        ...
        'guest-course-2': [
          ...
          {
            id: '4',
            question: 'Which lightsaber form emphasizes aggressive offense?',
            options: [
              'Form III (Soresu)',
            ...
            correct_answer: 1,
            explanation: 'Form V (Shien/Djem So) emphasizes aggressive offense and counter-attacks, turning defensive moves into offensive opportunities.'
          },
          {
            id: '5',
            question: 'What is the key principle behind Form IV (Ataru)?',
            options: [
              'Minimalist movements and efficiency',
              'Acrobatic mobility and kinetic combat',
              'Dual-weapon coordination',
              'Mental focus over physical technique'
            ],
            correct_answer: 1,
            explanation: 'Form IV (Ataru) emphasizes acrobatic mobility, using the Force to enhance physical capabilities for kinetic, fast-paced combat.'
          }
        ],
        'guest-course-3': [
          {
          ...
          {
            id: '4',
            question: 'What is the relationship between meditation and emotional control?',
            options: [
              'Meditation suppresses all emotions completely',
              'Meditation helps understand and manage emotions without being controlled by them',
              'Meditation only works for naturally calm personalities',
              'Meditation replaces the need for emotional awareness'
            ],
            correct_answer: 1,
            explanation: 'Meditation helps Jedi understand and manage their emotions without being controlled by them, maintaining balance between feeling and rational thought.'
          },
          {
            id: '5',
            question: 'How does mindfulness practice benefit Force sensitivity?',
            options: [
              'It increases raw Force power directly',
              'It enhances awareness and perception of Force currents',
              'It allows manipulation of others\' thoughts',
              'It provides immunity to dark side influences'
            ],
            correct_answer: 1,
            explanation: 'Mindfulness practice enhances awareness and perception, allowing Jedi to better sense and understand the subtle currents of the Force around them.'
          }
        ]
      };
  ...
  const validateQuestionData = (questionData: any): QuizQuestion | null => {
    try {
      ...
      if (!questionData || typeof questionData !== 'object') {
        console.warn('Invalid question data: not an object', questionData);
        return null;
      }
      ...
      if (
        ...
        console.warn('Invalid question data: missing or invalid fields', {
          id: !!id,
          question: typeof question === 'string' && question.length > 0,
          options: Array.isArray(options) && options.length === 4,
          correct_answer: typeof correct_answer === 'number' && correct_answer >= 0 && correct_answer <= 3,
          explanation: typeof explanation === 'string' && explanation.length > 0
        });
        return null;
      }
  ...
  return (
    <QuizErrorBoundary onError={handleError}>
      <div className="min-h-screen bg-gradient-to-br from-[#F5F7FA] to-[#E1E8F0] pl-64 pt-16">
        <div className="max-w-4xl mx-auto p-8">
          <motion.button
            onClick={() => navigate('/courses')}
